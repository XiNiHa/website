...
const Item: Component<Props> = props => {
  ...
  return (
    <li
      ...
      classList={{ 'xl:gap-2': !props.experience.data.stack }}
    >
      <div class="flex flex-col gap-1 print:break-inside-avoid-page">
        <div
          class="flex flex-wrap items-center gap-2 cursor-pointer w-fit mb-1 transition-colors duration-300 border-b border-b-transparent hover:border-b-gray-400 sm:flex-nowrap"
          onClick={() => setExpanded(prev => !prev)}
        >
          <h3 class="relative text-#333 flex flex-wrap items-center gap-2 text-xl xl:text-3xl">
      ...
      >
        <div
          ref={contentRef}
          class="hide-br-on-small text-lg xl:text-xl print:(text-base leading-[1.4])"
        >
          <p ref={fixedPartRef}>
          ...
          <Show when={props.experience.rendered}>
            {rendered => (
              <div
                ...
                class="flex flex-col font-body underline-a markdown-body"
              />
            )}
          </Show>
        </div>
